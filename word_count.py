...
def top_words(source, number):
    ...
    with open(source) as f:
        for line in f:
            ...
            for w in words:
                w_lower = w.lower()
                word_list.append(w_lower)
    word_list.sort()

    for word in word_list:
        if ht.contains_key(word) is False:
            ht.put(word, 1)
        else:
    ...
    for i in ht.get_buckets():
        cur = i.head
        while cur is not None:
            top_list[cur.key] = cur.value
            cur = cur.next

    top_list_list = [(k, v) for k, v in top_list.items()]

    top_list_list.sort(key=lambda x: x[1], reverse=True)

    return top_list_list[0:number]
