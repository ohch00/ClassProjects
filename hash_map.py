...
class HashMap:
    ...
    def resize_table(self, capacity):
        ...
        capacity = capacity
        for i in range(capacity):
            buckets2.append(LinkedList())
        for x in self._buckets:
            if x.head is not None:
                cur = x.head
                while cur is not None:
                    hash = self._hash_function(cur.key)
                    index = hash % capacity
                    x = buckets2[index]
                    x.add_front(cur.key, cur.value)
                    size += 1
                    cur = cur.next

        self._buckets = buckets2
        self.capacity = capacity
        self.size = size
    ...
    def table_load(self):
        ...
        for i in self._buckets:
            ...
            while cur is not None:
                counter += 1
                cur = cur.next
        load_factor = counter / self.capacity
        return load_factor
